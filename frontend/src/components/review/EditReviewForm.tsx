...
import { REVIEW_TAGS, REVIEW_TAG_LABELS, type ReviewTag, type SubmitReviewRequest, type Review } from '../../types/review';
...
/**
 * Form component for editing an existing bus review
 */
export const EditReviewForm: React.FC<EditReviewFormProps> = ({
  ...
  const [selectedTags, setSelectedTags] = useState<ReviewTag[]>((review.tags || []) as ReviewTag[]);
  const [travelDate, setTravelDate] = useState(review.travelDate || '');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await reviewService.editReview(review.id, request);
      onSuccess();
    } catch (err) {
      ...
      <div className="bg-white dark:bg-gray-800 rounded-t-2xl sm:rounded-xl shadow-2xl w-full sm:max-w-md max-h-[95vh] sm:max-h-[90vh] overflow-hidden flex flex-col animate-slide-up sm:animate-fade-in">
        ...
        <div className="flex items-center justify-between p-4 border-b dark:border-gray-700 sticky top-0 bg-white dark:bg-gray-800 z-10">
          <h3 className="text-lg font-semibold text-gray-900 dark:text-white">
            {t('review.editTitle', 'Update your review')}
          </h3>
          <button
            onClick={onCancel}
