...
import { getLocations, searchBuses } from '../../services/api';

// Mock environment utilities directly instead of requiring a module
jest.mock('../../utils/environment', () => ({
  getEnv: jest.fn().mockImplementation((key) => {
    switch (key) {
      case 'VITE_API_URL':
  ...
  getFeatureFlag: jest.fn().mockImplementation((key) => {
    return key === 'VITE_FEATURE_TRACKING' || 
