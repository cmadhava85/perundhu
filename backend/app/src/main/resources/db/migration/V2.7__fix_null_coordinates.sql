-- V2.7__fix_null_coordinates.sql
...
SET @constraint_exists = (SELECT COUNT(*) FROM INFORMATION_SCHEMA.TABLE_CONSTRAINTS 
    WHERE CONSTRAINT_NAME = 'chk_latitude_range' AND TABLE_SCHEMA = DATABASE());
...
SET @constraint_exists = (SELECT COUNT(*) FROM INFORMATION_SCHEMA.TABLE_CONSTRAINTS 
    WHERE CONSTRAINT_NAME = 'chk_longitude_range' AND TABLE_SCHEMA = DATABASE());
SET @sql = IF(@constraint_exists = 0, 
...
SET @index_exists = (SELECT COUNT(*) FROM INFORMATION_SCHEMA.STATISTICS 
    WHERE INDEX_NAME = 'idx_locations_coordinates' AND TABLE_SCHEMA = DATABASE());
