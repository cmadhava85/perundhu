...
public interface BusJpaRepository extends JpaRepository<BusJpaEntity, Long> {
        ...
        @Query("SELECT b FROM BusJpaEntity b WHERE b.fromLocation.id = :fromLocationId AND b.toLocation.id = :toLocationId AND (b.active = true OR b.active IS NULL)")
        List<BusJpaEntity> findByFromLocationIdAndToLocationId(@Param("fromLocationId") Long fromLocationId, @Param("toLocationId") Long toLocationId);

        @Query("SELECT b FROM BusJpaEntity b WHERE b.fromLocation.id = :fromLocationId AND (b.active = true OR b.active IS NULL)")
        List<BusJpaEntity> findByFromLocationId(@Param("fromLocationId") Long fromLocationId);

        @Query("SELECT b FROM BusJpaEntity b WHERE (b.fromLocation.id = :fromLocationId OR b.toLocation.id = :toLocationId) AND (b.active = true OR b.active IS NULL)")
        List<BusJpaEntity> findByFromLocationIdOrToLocationId(@Param("fromLocationId") Long fromLocationId, @Param("toLocationId") Long toLocationId);
        ...
        boolean existsByBusNumberAndFromAndToLocationsAndTiming(@Param("busNumber") String busNumber,
                        @Param("fromLocationName") String fromLocationName,
                        @Param("toLocationName") String toLocationName,
        ...
        @Query("SELECT DISTINCT b FROM BusJpaEntity b " +
                        ...
                        "AND (b.active = true OR b.active IS NULL)")
        List<BusJpaEntity> findBusesPassingThroughLocations(@Param("fromLocationId") Long fromLocationId,
                        @Param("toLocationId") Long toLocationId);
        ...
        @Query("SELECT DISTINCT b FROM BusJpaEntity b " +
                        ...
                        "AND (b.active = true OR b.active IS NULL) " +
                        "AND EXISTS (SELECT s3 FROM StopJpaEntity s3 WHERE s3.bus.id = b.id AND s3.stopOrder > s2.stopOrder)")
        List<BusJpaEntity> findBusesContinuingBeyondDestination(@Param("fromLocationId") Long fromLocationId,
                        @Param("toLocationId") Long toLocationId);
        ...
        @Query("SELECT b FROM BusJpaEntity b WHERE b.busNumber = :busNumber AND b.fromLocation.id = :fromLocationId AND b.toLocation.id = :toLocationId")
